...
import com.medicalcenterportal.services.FollowUpNologinService;

import io.swagger.v3.oas.annotations.parameters.RequestBody;
import jakarta.servlet.http.HttpServletRequest;

@Controller
@RequestMapping("/followupnologin")
public class FollowUpNoLoginController {
@Autowired
FollowUpNologinService followUpNologinService;
@Autowired 
FollowUpNoLoginRepository followUpNoLoginRepository;
@Autowired
private UserRepository userRepository;
@Autowired 
AppointmentNoLoginRepository appointmentRepo;
@Autowired 
AppointmentNoLoginService appointmentNoLoginRepository;
@Autowired 
...
public String listFollowUpAppointments(Model model, Principal principal) {
    ...
    List<FollowUpNoLogin> followUpAppointments = new ArrayList<>();
    LocalDate currentDate = LocalDate.now(); // Lấy ngày hiện tại

    if (isAdmin) {
        // Nếu người dùng có vai trò ADMIN, lấy tất cả cuộc hẹn Follow-Up
        followUpAppointments = followUpNologinService.findAll();
    } else if (isDoctor) {
        ...
        List<FollowUpNoLogin> doctorAppointments = followUpNologinService.findByDoctor(doctor);
        // Lọc ra các cuộc hẹn có appointmentDate là ngày hiện tại
        for (FollowUpNoLogin appointment : doctorAppointments) {
    ...
    Collections.sort(followUpAppointments, Comparator.comparing(FollowUpNoLogin::getCreateOn).reversed());

    // Thêm danh sách cuộc hẹn vào model
    model.addAttribute("followUpAppointments", followUpAppointments);

    return "/apps/followupNoLogin/listFollowUpNoLogin";
}
...
public String showEditAppointmentForm(@PathVariable("id") Long id, Model model) {
	...
	Optional<AppointmentNoLogin> appointmentOptional = appointmentNoLoginRepository.getAppointmentById(id);
	
	
	if (appointmentOptional.isPresent()) {
	    AppointmentNoLogin appointment = appointmentOptional.get();
	    ...
	    FollowUpNoLogin followUpAppointment = new FollowUpNoLogin();
	
	    followUpAppointment.setPatientUser(appointment);
	    
	    followUpAppointment.setSpecials(appointment.getSpecialty());

	    model.addAttribute("followUpAppointment", followUpAppointment);

	    // Trả về view cho việc chỉnh sửa thông tin cuộc hẹn
	    return "newFollowUpNoLogin";
	} else {
...
public String showEditFollowUpAppointment(@PathVariable("id") Long id, Model model) {
    ...
    Optional<FollowUpNoLogin> followUpAppointmentOptional = followUpNologinService.findById(id);
    
    // Kiểm tra xem followUpAppointmentOptional có giá trị không trước khi truy cập
    if (followUpAppointmentOptional.isPresent()) {
        FollowUpNoLogin followUpAppointment = followUpAppointmentOptional.get();
        ...
        Specialty specialty = followUpAppointment.getSpecials();
...
@PostMapping("/update")
public String updateAppointment(HttpServletRequest request, @ModelAttribute("followUpAppointment") FollowUpNoLogin followUpAppointment, Model model) {
    String idStr = request.getParameter("id");
    Long appointmentId = Long.valueOf(idStr);
    
    // Lấy thông tin cuộc hẹn hiện tại từ cơ sở dữ liệu
    FollowUpNoLogin currentAppointment = followUpNologinService.findById(appointmentId)
    ...
    followUpNologinService.updateFollowUpNoLogin(appointmentId, followUpAppointment);
    
    return "redirect:/followupnologin/list";
}
@PostMapping("/save")
public String saveFollowUpAppointment(@RequestParam("patientUserId") Long patientUserId,
    ...
    try {
        ...
        Optional<AppointmentNoLogin> patientUserOptional = appointmentNoLoginRepository.getAppointmentById(patientUserId);
        // Lấy thông tin của chuyên khoa từ cơ sở dữ liệu bằng id
        Specialty specialty = appointmentRepo.findSpecialtyById(specialtyId);

        // Kiểm tra xem liệu thông tin của bệnh nhân và chuyên khoa có tồn tại không
        if (patientUserOptional.isPresent() && specialty != null) {
            AppointmentNoLogin patientUser = patientUserOptional.get();
            ...
            followUpAppointment.setSpecials(specialty);
            ...
            followUpNoLoginRepository.save(followUpAppointment);
            ...
            return "redirect:/error"; // Hoặc thực hiện xử lý lỗi khác tùy thuộc vào yêu cầu của ứng dụng
        }
    } catch (Exception e) {
        // Xử lý lỗi nếu có
        return "redirect:/error"; // Hoặc thực hiện xử lý lỗi khác tùy thuộc vào yêu cầu của ứng dụng
...
public String showFollowUp(@PathVariable("id") Long id, Model model) {
    // Lấy thông tin cuộc hẹn từ ID
    Optional<FollowUpNoLogin> followUpAppointmentOptional = followUpNoLoginRepository.findById(id);
    ...
    if (followUpAppointmentOptional.isPresent()) {
        ...
        FollowUpNoLogin formFollowUpAppointment = new FollowUpNoLogin();
        formFollowUpAppointment.setPatientUser(followUpAppointment.getPatientUser());
        formFollowUpAppointment.setSpecials(followUpAppointment.getSpecials());

        model.addAttribute("followUpAppointment", formFollowUpAppointment);

        return "newFollowUpNoLogin"; // Trả về view để hiển thị form
    } else {
        // Xử lý trường hợp không tìm thấy cuộc hẹn
        return "redirect:/error"; // Chuyển hướng đến trang lỗi
    }
